...
12 yes/no questions for reviewing a memory scheme against the practices
...
1. Does every stored entry belong to a named kind (for example fact,
   decision, or user), rather than one undifferentiated list?
...
6. Is there a deduplication check that runs before a write, so
   near-duplicate entries get merged or updated instead of piling up?
7. Is there a check that blocks credentials, tokens, or other secrets
   from being written into memory, even as an incidental part of a larger
   claim?
...
8. Does every entry have at least one defined path to deletion (user
   ...
   being overwritten?
9. When a fact changes, is the old value explicitly retired
   (supersession) rather than left live alongside the new one?
...
11. Is retrieved memory content passed to the agent clearly delimited
    from system instructions and user requests, rather than concatenated
    into the instruction stream unmarked?
12. If a stored entry contains something that reads as an instruction,
    such as `always do X` or `ignore Y`, does the agent treat that as a
    claim to weigh rather than a command to follow?

## Scoring

12 yes answers means the scheme matches the guide. Each no points at
a specific section of the README to revisit; the questions are ordered to
match, so a no on question *n* maps to the section it was drawn from.
